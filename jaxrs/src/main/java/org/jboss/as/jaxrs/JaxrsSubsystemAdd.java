...
class JaxrsSubsystemAdd extends AbstractBoottimeAddStepHandler {
    ...
    private static JaxrsServerConfig createServerConfig(ModelNode configuration, OperationContext context) throws OperationFailedException {
        ...
        if (configuration.hasDefined(JaxrsConstants.RESTEASY_PREFERJACKSONOVERJSONB)) {
            config.setResteasyPreferJacksonOverJsonB(JaxrsAttribute.RESTEASY_PREFERJACKSONOVERJSONB.resolveModelAttribute(context, configuration));
        }
        ...
        if (configuration.hasDefined(JaxrsConstants.RESTEASY_RESOURCES)) {
            config.setResteasyResources(JaxrsAttribute.RESTEASY_RESOURCES.resolveModelAttribute(context, configuration));
        }
